...
class RancherService:
    ...
    def get_certificate(self):
        ...
        while not done:
            try:
                ...
                print("ERROR: Cannot connect to URL: {0} for method {1}. "
                      "Full error: {2}".format(url, "get_certificate", str(e)))
    ...
    def delete_cert(self, server):
        ...
        url = "{0}/projects/{1]/certificates/{2}".format(RANCHER_URL,
                                                         self.get_project_id(),
                                                         self.get_certificate_id(server))
        done = False
        while not done:
            try:
                ...
                print("ERROR: Cannot connect to URL: {0} for method {1}. "
                      "Full error: {2}".format(url, "delete_cert", str(e)))
                print("ERROR: Trying to reconnect in {0} seconds".format(CONNECT_WAIT))
                time.sleep(CONNECT_WAIT)
                continue
            # done with exceptions
    ...
    def check_cert_files_exist(self, server):
        ...
        return os.path.isdir(cert_dir) and os.path.isfile(cert) and \
               os.path.isfile(privkey) and os.path.isfile(chain) and os.path.isfile(fullchain)
    ...
    def local_cert_expired(self, cert_string):
        """
        if there is a certificate in LETSENCRYPT_ROOTDIR, we should check that it is itself
    ...
    def post_cert(self, server):
        ...
        if self.check_cert_files_exist(server):
            json_structure = {'certChain': self.read_chain(server), 'cert': self.read_cert(server),
                              'key': self.read_privkey(server), 'type': 'certificate', 'name': server, 'created': None,
                              'description': None, 'kind': None, 'removed': None, 'uuid': None}

            headers = {'Content-Type': 'application/json'}
            done = False
            while not done:
                try:
                    r = request_type(url=url, data=json.dumps(json_structure), headers=headers,
                                     auth=self.auth(), timeout=CONNECT_TIMEOUT)
                except requests.exceptions.ConnectionError as e:
                    print("ERROR: Cannot connect to URL: {0} for method {1}. "
                          "Full error: {2}".format(url, "post_cert", str(e)))
                    print("ERROR: Trying to reconnect in {0} seconds".format(CONNECT_WAIT))
                    time.sleep(CONNECT_WAIT)
                    continue
    ...
    def get_project_id(self):
        ...
        while not done:
            try:
                ...
                print("ERROR: Cannot connect to URL: {0} for method {1}. "
                      "Full error: {2}".format(url, "get_project_id", str(e)))
                print("ERROR: Trying to reconnect in {0} seconds".format(CONNECT_WAIT))
                time.sleep(CONNECT_WAIT)
                continue
            ...
            for host in self.parse_servernames():
                if self.hostname_resolves(host):
                    ...
                    if self.port_open(host, HOST_CHECK_PORT):
                        ...
                        # a 301 redirect. Also, if we get a 503 service unavailable status code there is no l
                        # ets-encrypt nginx container working, and we should continue to wait and NOT requests Let's
                        # Encrypt certificates yet.
                        ...
                        if r.status_code != 503 and r.status_code != 301:
                            print("\t\tINFO: OK, got HTTP status code ({0}) for ({1})".format(r.status_code,
                                                                                              host))
                            done = True
                        else:
                            print("\t\tINFO: Received bad HTTP status code ({0}) from ({1})".format(r.status_code,
                                                                                                    host))
                            done = False
                    else:
